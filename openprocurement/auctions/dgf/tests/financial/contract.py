...
from openprocurement.auctions.dgf.tests.base import BaseFinancialAuctionWebTest, test_financial_auction_data, test_financial_bids, test_lots, test_financial_organization


class AuctionContractResourceTest(BaseFinancialAuctionWebTest):
    ...
    def setUp(self):
        ...
        response = self.app.post_json('/auctions/{}/awards'.format(
            self.auction_id), {'data': {'suppliers': [test_financial_organization], 'status': 'pending', 'bid_id': self.initial_bids[0]['id'], 'value': test_financial_auction_data["value"], 'items': test_financial_auction_data["items"]}})
...
class Auction2LotContractResourceTest(BaseFinancialAuctionWebTest):
    initial_status = 'active.qualification'
    initial_bids = test_financial_bids
...
class AuctionContractDocumentResourceTest(BaseFinancialAuctionWebTest):
    #initial_data = auction_data
    initial_status = 'active.qualification'
    initial_bids = test_financial_bids
...
@unittest.skip("option not available")
class Auction2LotContractDocumentResourceTest(BaseFinancialAuctionWebTest):
    initial_status = 'active.qualification'
    initial_bids = test_financial_bids
    initial_lots = 2 * test_lots
