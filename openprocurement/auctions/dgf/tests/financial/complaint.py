...
from openprocurement.auctions.dgf.tests.base import BaseFinancialAuctionWebTest, test_financial_auction_data, test_lots, test_financial_organization


class AuctionComplaintResourceTest(BaseFinancialAuctionWebTest):
    ...
    def test_create_auction_complaint_invalid(self):
        ...
        response = self.app.post_json(request_path, {'data': {'title': 'complaint title', 'description': 'complaint description', 'author': test_financial_organization, 'relatedLot': '0' * 32}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
    ...
    def test_create_auction_complaint(self):
        response = self.app.post_json('/auctions/{}/complaints'.format(
            self.auction_id), {'data': {'title': 'complaint title', 'description': 'complaint description', 'author': test_financial_organization, 'status': 'claim'}})
        ...
        response = self.app.post_json('/auctions/{}/complaints'.format(
            self.auction_id), {'data': {'title': 'complaint title', 'description': 'complaint description', 'author': test_financial_organization}}, status=403)
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.content_type, 'application/json')
    ...
    def test_patch_auction_complaint(self):
        response = self.app.post_json('/auctions/{}/complaints'.format(
            self.auction_id), {'data': {'title': 'complaint title', 'description': 'complaint description', 'author': test_financial_organization}})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
        complaint = response.json['data']
        owner_token = response.json['access']['token']
...
class AuctionComplaintDocumentResourceTest(BaseFinancialAuctionWebTest):
