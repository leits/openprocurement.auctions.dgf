...
from openprocurement.auctions.dgf.tests.base import BaseWebTest, BaseFinancialAuctionWebTest, test_financial_auction_data, test_lots, test_financial_organization


@unittest.skip("option not available")
class AuctionLotResourceTest(BaseFinancialAuctionWebTest):
    ...
    def test_create_auction_lot(self):
        response = self.app.post_json('/auctions/{}/lots'.format(self.auction_id), {'data': test_lots[0]})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
        ...
        response = self.app.post_json('/auctions/{}/lots'.format(self.auction_id), {'data': test_lots[0]}, status=403)
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['errors'][0]["description"], "Can't add lot in current (active.tendering) auction status")

    def test_patch_auction_lot(self):
        response = self.app.post_json('/auctions/{}/lots'.format(self.auction_id), {'data': test_lots[0]})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
        lot = response.json['data']
    ...
    def test_patch_auction_currency(self):
        # create lot
        response = self.app.post_json('/auctions/{}/lots'.format(self.auction_id), {'data': test_lots[0]})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
    ...
    def test_get_auction_lots(self):
        response = self.app.post_json('/auctions/{}/lots'.format(self.auction_id), {'data': test_lots[0]})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
        lot = response.json['data']
    ...
    def test_delete_auction_lot(self):
        response = self.app.post_json('/auctions/{}/lots'.format(self.auction_id), {'data': test_lots[0]})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
        lot = response.json['data']
    ...
    def test_auction_lot_guarantee(self):
        data = deepcopy(test_financial_auction_data)
...
@unittest.skip("option not available")
class AuctionLotFeatureResourceTest(BaseFinancialAuctionWebTest):
    ...
    def test_auction_features_invalid(self):
        request_path = '/auctions/{}'.format(self.auction_id)
        data = test_financial_auction_data.copy()
...
@unittest.skip("option not available")
class AuctionLotBidderResourceTest(BaseFinancialAuctionWebTest):
    initial_status = 'active.tendering'
    initial_lots = test_lots

    def test_create_auction_bidder_invalid(self):
        request_path = '/auctions/{}/bids'.format(self.auction_id)
        response = self.app.post_json(request_path, {'data': {'tenderers': [test_financial_organization]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 500}}]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 500}, 'relatedLot': "0" * 32}]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 50}, 'relatedLot': self.initial_lots[0]['id']}]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 500, 'valueAddedTaxIncluded': False}, 'relatedLot': self.initial_lots[0]['id']}]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 500, 'currency': "USD"}, 'relatedLot': self.initial_lots[0]['id']}]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': [test_financial_organization], "value": {"amount": 500}, 'lotValues': [{"value": {"amount": 500}, 'relatedLot': self.initial_lots[0]['id']}]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': test_financial_organization, 'lotValues': [{"value": {"amount": 500}, 'relatedLot': self.initial_lots[0]['id']}]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
    ...
    def test_patch_auction_bidder(self):
        ...
        response = self.app.post_json('/auctions/{}/bids'.format(self.auction_id), {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 500}, 'relatedLot': lot_id}]}})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
        ...
        response = self.app.patch_json('/auctions/{}/bids/{}'.format(self.auction_id, bidder['id']), {"data": {'lotValues': [{"value": {"amount": 500}, 'relatedLot': lot_id}], 'tenderers': [test_financial_organization]}})
        self.assertEqual(response.status, '200 OK')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['data']['lotValues'][0]['date'], lot['date'])
...
@unittest.skip("option not available")
class AuctionLotFeatureBidderResourceTest(BaseFinancialAuctionWebTest):
    initial_lots = test_lots
    ...
    def test_create_auction_bidder_invalid(self):
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 50}, 'relatedLot': self.lot_id}]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 500, 'valueAddedTaxIncluded': False}, 'relatedLot': self.lot_id}]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 500, 'currency': "USD"}, 'relatedLot': self.lot_id}]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': test_financial_organization, 'lotValues': [{"value": {"amount": 500}, 'relatedLot': self.lot_id}]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 500}, 'relatedLot': self.lot_id}], 'parameters': [{"code": "code_item", "value": 0.01}]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 500}, 'relatedLot': self.lot_id}], 'parameters': [{"code": "code_invalid", "value": 0.01}]}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 500}, 'relatedLot': self.lot_id}], 'parameters': [
    ...
    def test_create_auction_bidder(self):
        ...
        self.assertEqual(response.content_type, 'application/json')
        bidder = response.json['data']
        self.assertEqual(bidder['tenderers'][0]['name'], test_financial_organization['name'])
...
@unittest.skip("option not available")
class AuctionLotProcessTest(BaseFinancialAuctionWebTest):
    setUp = BaseWebTest.setUp

    def test_1lot_0bid(self):
        self.app.authorization = ('Basic', ('broker', ''))
        # create auction
        response = self.app.post_json('/auctions', {"data": test_financial_auction_data})
        auction_id = self.auction_id = response.json['data']['id']
        owner_token = response.json['access']['token']
        # add lot
        response = self.app.post_json('/auctions/{}/lots?acc_token={}'.format(auction_id, owner_token), {'data': test_lots[0]})
        self.assertEqual(response.status, '201 Created')
        lot_id = response.json['data']['id']
        # add relatedLot for item
        response = self.app.patch_json('/auctions/{}?acc_token={}'.format(auction_id, owner_token), {"data": {"items": [{'relatedLot': lot_id}]}})
        self.assertEqual(response.status, '200 OK')
    ...
    def test_1lot_1bid(self):
        ...
        response = self.app.post_json('/auctions/{}/bids'.format(auction_id),
                                      {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 500}, 'relatedLot': lot_id}]}})
    ...
    def test_1lot_2bid(self):
        ...
        self.app.authorization = ('Basic', ('broker', ''))
        response = self.app.post_json('/auctions/{}/bids'.format(auction_id),
                                      {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 450}, 'relatedLot': lot_id}]}})
        bid_id = response.json['data']['id']
        bid_token = response.json['access']['token']
        # create second bid
        self.app.authorization = ('Basic', ('broker', ''))
        response = self.app.post_json('/auctions/{}/bids'.format(auction_id),
                                      {'data': {'tenderers': [test_financial_organization], 'lotValues': [{"value": {"amount": 475}, 'relatedLot': lot_id}]}})
        ...
        self.assertEqual(response.json['data']['lotValues'][0]['participationUrl'], 'https://auction.auction.url/for_bid/{}'.format(bid_id))
        # posting auction results
        self.app.authorization = ('Basic', ('auction', ''))
        response = self.app.post_json('/auctions/{}/auction/{}'.format(auction_id, lot_id), {'data': {'bids': auction_bids_data}})
        # get awards
        self.app.authorization = ('Basic', ('broker', ''))
        response = self.app.get('/auctions/{}/awards?acc_token={}'.format(auction_id, owner_token))
        # get pending award
    ...
    def test_2lot_0bid(self):
        ...
        # add item
        response = self.app.patch_json('/auctions/{}?acc_token={}'.format(auction_id, owner_token), {"data": {"items": [test_financial_auction_data['items'][0] for i in lots]}})
        # add relatedLot for item
        response = self.app.patch_json('/auctions/{}?acc_token={}'.format(auction_id, owner_token), {"data": {"items": [{'relatedLot': i} for i in lots]}})
        self.assertEqual(response.status, '200 OK')
        # switch to active.tendering
    ...
    def test_2lot_2bid_0com_1can_before_auction(self):
        ...
        response = self.app.patch_json('/auctions/{}?acc_token={}'.format(auction_id, owner_token),
                                       {"data": {"items": [test_financial_auction_data['items'][0] for i in lots]}})
        # add relatedLot for item
        response = self.app.patch_json('/auctions/{}?acc_token={}'.format(auction_id, owner_token),
        ...
        self.app.authorization = ('Basic', ('broker', ''))
        response = self.app.post_json('/auctions/{}/bids'.format(auction_id),
                                      {'data': {'tenderers': [test_financial_organization], 'lotValues': [
    ...
    def test_2lot_1bid_0com_1can(self):
        ...
        # create bid
        self.app.authorization = ('Basic', ('broker', ''))
        response = self.app.post_json('/auctions/{}/bids'.format(auction_id), {'data': {'tenderers': [test_financial_organization], 'lotValues': [
            {"value": {"amount": 500}, 'relatedLot': lot_id}
            for lot_id in lots
        ]}})
        # switch to active.qualification
        ...
        award_id = [i['id'] for i in response.json['data'] if i['status'] == 'pending' and i['lotID'] == lot_id][0]
        # set award as unsuccessful
        self.app.patch_json('/auctions/{}/awards/{}?acc_token={}'.format(auction_id, award_id, owner_token), {"data": {"status": "unsuccessful"}})
        # after stand slill period
        self.set_status('complete', {'status': 'active.awarded'})
        # time travel
        auction = self.db.get(auction_id)
