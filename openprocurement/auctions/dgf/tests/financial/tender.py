...
from openprocurement.auctions.dgf.tests.base import test_financial_auction_data, test_financial_organization, BaseWebTest, BaseFinancialAuctionWebTest
...
class AuctionTest(BaseWebTest):
    ...
    def test_simple_add_auction(self):
        ...
        u = Auction(test_financial_auction_data)
        u.auctionID = "UA-EA-X"
...
class AuctionResourceTest(BaseWebTest):
    ...
    def test_listing(self):
        ...
        test_financial_auction_data2 = test_financial_auction_data.copy()
        test_financial_auction_data2['mode'] = 'test'
        response = self.app.post_json('/auctions', {'data': test_financial_auction_data2})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
    ...
    def test_create_auction_invalid(self):
        ...
        response = self.app.post_json(request_path, {'data': {'invalid_field': 'invalid_value', 'procurementMethodType': test_financial_auction_data['procurementMethodType']}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'value': 'invalid_value', 'procurementMethodType': test_financial_auction_data['procurementMethodType']}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'procurementMethod': 'invalid_value', 'procurementMethodType': test_financial_auction_data['procurementMethodType']}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        ...
        response = self.app.post_json(request_path, {'data': {'enquiryPeriod': {'endDate': 'invalid_value'}, 'procurementMethodType': test_financial_auction_data['procurementMethodType']}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.post_json(request_path, {'data': {'enquiryPeriod': {'endDate': '9999-12-31T23:59:59.999999'}, 'procurementMethodType': test_financial_auction_data['procurementMethodType']}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        data = test_financial_auction_data['tenderPeriod']
        test_financial_auction_data['tenderPeriod'] = {'startDate': '2014-10-31T00:00:00', 'endDate': '2014-10-01T00:00:00'}
        response = self.app.post_json(request_path, {'data': test_financial_auction_data}, status=422)
        test_financial_auction_data['tenderPeriod'] = data
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        #data = test_financial_auction_data['tenderPeriod']
        #test_financial_auction_data['tenderPeriod'] = {'startDate': '2014-10-31T00:00:00', 'endDate': '2015-10-01T00:00:00'}
        #response = self.app.post_json(request_path, {'data': test_financial_auction_data}, status=422)
        #test_financial_auction_data['tenderPeriod'] = data
        #self.assertEqual(response.status, '422 Unprocessable Entity')
        #self.assertEqual(response.content_type, 'application/json')
        ...
        now = get_now()
        test_financial_auction_data['awardPeriod'] = {'startDate': now.isoformat(), 'endDate': now.isoformat()}
        response = self.app.post_json(request_path, {'data': test_financial_auction_data}, status=422)
        del test_financial_auction_data['awardPeriod']
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        test_financial_auction_data['auctionPeriod'] = {'startDate': (now + timedelta(days=15)).isoformat(), 'endDate': (now + timedelta(days=15)).isoformat()}
        test_financial_auction_data['awardPeriod'] = {'startDate': (now + timedelta(days=14)).isoformat(), 'endDate': (now + timedelta(days=14)).isoformat()}
        response = self.app.post_json(request_path, {'data': test_financial_auction_data}, status=422)
        del test_financial_auction_data['auctionPeriod']
        del test_financial_auction_data['awardPeriod']
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        ...
        data = test_financial_auction_data['minimalStep']
        test_financial_auction_data['minimalStep'] = {'amount': '1000.0'}
        response = self.app.post_json(request_path, {'data': test_financial_auction_data}, status=422)
        test_financial_auction_data['minimalStep'] = data
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        data = test_financial_auction_data['minimalStep']
        test_financial_auction_data['minimalStep'] = {'amount': '100.0', 'valueAddedTaxIncluded': False}
        response = self.app.post_json(request_path, {'data': test_financial_auction_data}, status=422)
        test_financial_auction_data['minimalStep'] = data
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        ...
        data = test_financial_auction_data['minimalStep']
        test_financial_auction_data['minimalStep'] = {'amount': '100.0', 'currency': "USD"}
        response = self.app.post_json(request_path, {'data': test_financial_auction_data}, status=422)
        test_financial_auction_data['minimalStep'] = data
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        ...
        auction_data = deepcopy(test_financial_auction_data)
        ...
        data = test_financial_auction_data["items"][0]["additionalClassifications"][0]["scheme"]
        test_financial_auction_data["items"][0]["additionalClassifications"][0]["scheme"] = 'Не ДКПП'
        response = self.app.post_json(request_path, {'data': test_financial_auction_data}, status=422)
        test_financial_auction_data["items"][0]["additionalClassifications"][0]["scheme"] = data
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        data = test_financial_organization["contactPoint"]["telephone"]
        del test_financial_organization["contactPoint"]["telephone"]
        response = self.app.post_json(request_path, {'data': test_financial_auction_data}, status=422)
        test_financial_organization["contactPoint"]["telephone"] = data
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        data = test_financial_auction_data["items"][0].copy()
        classification = data['classification'].copy()
        classification["id"] = u'66113000-5'
        data['classification'] = classification
        test_financial_auction_data["items"] = [test_financial_auction_data["items"][0], data]
        response = self.app.post_json(request_path, {'data': test_financial_auction_data}, status=422)
        test_financial_auction_data["items"] = test_financial_auction_data["items"][:1]
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        procuringEntity = test_financial_auction_data["procuringEntity"]
        data = test_financial_auction_data["procuringEntity"].copy()
        del data['kind']
        test_financial_auction_data["procuringEntity"] = data
        response = self.app.post_json(request_path, {'data': test_financial_auction_data}, status=201)
        test_financial_auction_data["procuringEntity"] = procuringEntity
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
    ...
    def test_create_auction(self):
        ...
        self.assertEqual(response.content_type, 'application/json')
        auction = response.json['data']
        self.assertEqual(set(auction) - set(test_financial_auction_data), set([
        ...
        response = self.app.post_json('/auctions?opt_jsonp=callback', {"data": test_financial_auction_data})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/javascript')
        self.assertIn('callback({"', response.body)

        response = self.app.post_json('/auctions?opt_pretty=1', {"data": test_financial_auction_data})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
        self.assertIn('{\n    "', response.body)

        response = self.app.post_json('/auctions', {"data": test_financial_auction_data, "options": {"pretty": True}})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
        self.assertIn('{\n    "', response.body)
    ...
    def test_patch_auction(self):
        ...
        response = self.app.patch_json('/auctions/{}'.format(
            auction['id']), {'data': {'items': [test_financial_auction_data['items'][0]]}})
        self.assertEqual(response.status, '200 OK')
        self.assertEqual(response.content_type, 'application/json')
    ...
    def test_auction_Administrator_change(self):
        ...
        response = self.app.post_json('/auctions/{}/questions'.format(auction['id']), {'data': {'title': 'question title', 'description': 'question description', 'author': test_financial_organization}})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
...
class AuctionProcessTest(BaseFinancialAuctionWebTest):
    ...
    def test_invalid_auction_conditions(self):
        ...
        response = self.app.post_json('/auctions/{}/complaints'.format(auction_id),
                                      {'data': {'title': 'invalid conditions', 'description': 'description', 'author': test_financial_organization, 'status': 'claim'}})
    ...
    def test_one_valid_bid_auction(self):
        ...
        response = self.app.post_json('/auctions/{}/bids'.format(auction_id),
                                      {'data': {'tenderers': [test_financial_organization], "value": {"amount": 500}, 'selfQualified': True}})
    ...
    def test_first_bid_auction(self):
        ...
        self.app.authorization = ('Basic', ('broker', ''))
        response = self.app.post_json('/auctions/{}/bids'.format(auction_id),
                                      {'data': {'tenderers': [test_financial_organization], "value": {"amount": 450}, 'selfQualified': True}})
        ...
        response = self.app.post_json('/auctions/{}/awards/{}/complaints?acc_token={}'.format(auction_id, award_id, bid_token),
                                      {'data': {'title': 'complaint title', 'description': 'complaint description', 'author': test_financial_organization, 'status': 'claim'}})
        complaint_id = response.json['data']['id']
        complaint_owner_token = response.json['access']['token']
        # create first award complaint #2
        response = self.app.post_json('/auctions/{}/awards/{}/complaints?acc_token={}'.format(auction_id, award_id, bid_token),
                                      {'data': {'title': 'complaint title', 'description': 'complaint description', 'author': test_financial_organization}})
        # answering claim
