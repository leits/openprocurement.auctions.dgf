...
def disqulify_last_award(auction):
    ...
    award['complaintPeriod']['endDate'] = get_now().isoformat()


def migrate_actual_award(auction):
    award = [a for a in auction["awards"] if a['status'] in ['active', 'pending']][0]
...
def from0to1(registry):
    ...
    for i in results:
        ...
        if all(a['status'] in ['cancelled', 'unsuccessful'] for a in awards):
